if (location.hostname.indexOf("searchtechnologies") > -1) {
    ...
    document.write("<style type='text/css'> .smallField { display:none ;}  .result-list{display:none;} .smallField>span {display:none;} .smallField>br{display:none;}  #f_categories>dl>dd {display:none; } #f_categories>div>a{display:none;} #f_year { display:none ;} #f_type {display:none;} #f_author {display:none} </style>");
    ...
    $(document).ready(function () {
        ...
        //$('#f_categories div').prepend("<div ><img  id='loader' src='http://www.ajaxload.info/cache/FF/FF/FF/00/00/00/6-1.gif' height='12' width='12'><font color='#808080'> Loading Categories...</font></div>"); //Show loading till get the category list from the proxy
        ...
        $('#f_categories div').load(location.host + '/?proxyReq=newCategories&timeStamp=' + new Date().getTime()+"&url="+encodeURI(location.href));
        var categoryChecker = setInterval(function () {  //Hide the results till the category list shows
            ...
            if ($('#f_categories>div>dl>dd').eq(0).html()) {
                console.log($('#f_categories>div>dl>dd').eq(0).html());
